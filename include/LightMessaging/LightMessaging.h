...
static inline mach_port_t LMMessageGetReplyPort(LMMessage *message)
{
	return message->head.msgh_remote_port;
}

static inline SInt32 LMMessageGetId(LMMessage *message)
{
	return message->head.msgh_id;
}
...
static inline mach_msg_return_t LMMachMsg(LMConnection *connection, mach_msg_header_t *msg, mach_msg_option_t option, mach_msg_size_t send_size, mach_msg_size_t rcv_size, mach_port_name_t rcv_name, mach_msg_timeout_t timeout, mach_port_name_t notify)
{
	for (;;) {
		...
		if (connection->serverPort == MACH_PORT_NULL) {
			// Lookup remote port
			bool useRocketBootstrap = sandbox_check(getpid(), "mach-lookup", SANDBOX_FILTER_LOCAL_NAME | SANDBOX_CHECK_NO_REPORT, connection->serverName);
			mach_port_t selfTask = mach_task_self();
			mach_port_t bootstrap = MACH_PORT_NULL;
			task_get_bootstrap_port(selfTask, &bootstrap);
			if (useRocketBootstrap) {
...
static inline kern_return_t LMStartServiceWithUserInfo(name_t serverName, bool unlockServer, CFRunLoopRef runLoop, CFMachPortCallBack callback, void *userInfo)
{
...
static inline kern_return_t LMStartService(name_t serverName, bool unlockServer, CFRunLoopRef runLoop, CFMachPortCallBack callback)
{
	return LMStartServiceWithUserInfo(serverName, unlockServer, runLoop, callback, NULL);
}
...
static inline bool LMDataWithSizeIsValidMessageOrEmpty(const void *data, CFIndex size, bool *isEmpty)
{
	if (size == sizeof(mach_msg_header_t) + sizeof(mach_msg_body_t)) {
		if (isEmpty)
			*isEmpty = true;
		...
		if (isEmpty)
			*isEmpty = false;
		return LMDataWithSizeIsValidMessage(data, size);
	}
}
...
static inline kern_return_t LMConnectionSendOneWayData(LMConnectionRef connection, SInt32 messageId, CFDataRef data)
{
	if (data)
		return LMConnectionSendOneWay(connection, messageId, CFDataGetBytePtr(data), CFDataGetLength(data));
	else
		return LMConnectionSendOneWay(connection, messageId, NULL, 0);
}
...
static inline kern_return_t LMConnectionSendOneWayPropertyList(LMConnectionRef connection, SInt32 messageId, id propertyList)
{
	return LMConnectionSendOneWayData(connection, messageId, propertyList ? (__bridge CFDataRef)[NSPropertyListSerialization dataFromPropertyList:propertyList format:NSPropertyListBinaryFormat_v1_0 errorDescription:NULL] : NULL);
}
...
{
	return LMConnectionSendTwoWayData(connection, messageId, propertyList ? (__bridge CFDataRef)[NSPropertyListSerialization dataFromPropertyList:propertyList format:NSPropertyListBinaryFormat_v1_0 errorDescription:NULL] : NULL, buffer);
}
...
static inline id LMResponseConsumePropertyList(LMResponseBuffer *buffer)
{
	...
	if (length) {
		...
		result = [NSPropertyListSerialization propertyListFromData:(__bridge NSData *)data mutabilityOption:0 format:NULL errorDescription:NULL];
		CFRelease(data);
	} else {
		result = nil;
	}
